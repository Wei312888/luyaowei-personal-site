...
async function startChat(db, res, message, history) {
  ...
  try {
    ...
    if (apiKey) await streamFromAgent(db, content, message, history, apiKey, controller, send, finish, () => closed);
    else await streamOffline(content, message, send, finish, () => closed);
  } catch (e) {
...
async function streamFromAgent(db, content, message, history, apiKey, controller, send, finish, isClosed) {
...
async function streamOffline(content, message, send, finish, isClosed) {
