...
function migrate(db) {
  ...
  // 5) 奖项名次勘误（2026-09 站主确认：AI 大赛=二等奖、电赛=省级二等奖；仅当仍是旧错值时改写）
  migrateHonorsCorrection(db);
  // 6) quickQuestions：seed 新增问题增量合并（append 缺失项，不覆盖既有顺序/自定义）
  migrateQuickQuestions(db, seed);
}
...
function migrateHonorsCorrection(db) {
  const rows = db.prepare("SELECT id, data FROM items WHERE category = 'honors'").all();
  const upd = db.prepare('UPDATE items SET data = ?, updated_at = ? WHERE id = ?');
  const now = new Date().toISOString();
  for (const row of rows) {
    let data;
    ...
    if (fix) upd.run(JSON.stringify({ ...data, detail: fix.right }), now, row.id);
  }
}

// quickQuestions 增量合并：seed 新增问题 append 到既有列表（保留站主自定义顺序，幂等）
function migrateQuickQuestions(db, seed) {
  if (!Array.isArray(seed.quickQuestions) || !seed.quickQuestions.length) return;
  const row = db.prepare('SELECT value FROM kv WHERE key = ?').get('quick_questions_json');
  if (!row) {
    kvSet(db, 'quick_questions_json', JSON.stringify(seed.quickQuestions));
    return;
  }
  try {
    ...
    let changed = false;
    for (const q of seed.quickQuestions) {
      if (!merged.includes(q)) { merged.push(q); changed = true; }
    }
    if (changed) kvSet(db, 'quick_questions_json', JSON.stringify(merged));
  } catch { /* 解析失败保持现状 */ }
}
